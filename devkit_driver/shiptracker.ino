...
static const char *psk_key ="xxxxxxxxxxxxx";;
...
void loop() {
  ...
  if ((last_send == 0) || (millis() - last_send >= SEND_INTERVAL)) {
    ...
    last_send = millis();
    float latitude = data.position.latitude_minutes;
    float longitude = data.position.longitude_minutes;
    ...
    LOG(L_INFO, "GPS Location: [%4.2f] degrees %s [%4.2f] degrees %s \n", latitude, north_south.c_str(), longitude, east_west.c_str());
    char commandText[512];
    snprintf(commandText, 512, "Current humidity [%4.2f] and current temp [%4.2f]. Latitude [%4.2f] %s , Longitude [%4.2f] %s", humidity, temperature, latitude, north_south.c_str(), longitude, east_west.c_str());
